...
;========================================================================================
	...
	Class BrowserBase {
		...
		ExcludeTasks	:=["трейд 12", "trade12 отзывы", "trade12 обман", "trade12", "трейд12", "национальная юридическая служба", "амулекс","ifс отзывы сотрудников", "ifc отзывы", "hotwheelspromo.by", "trade 12 отзывы" , "национальна юридическая служба", "брокер trade 12 развод", "freshforex", "фрешфорекс", "мас моторс отзывы", "www trade 12 com" , "роза в колбе", "http://trade12.com", "доска обрезная", "отзывы о матрасах Dreamline", "blackbook", "дримлайн матрас положительный отзыв", "пфк пребенд", "пребенд", "купить шкафчики для раздевалки", "юрист недвижимость", "Кондиционеры гри", "купить окна в севастополе", "кондиционеры севастополь","Сейфеддин", "yolkki village", "купить софосбувир", "купить велпанат", "стол тумба кресло"]
	...
	;Choose the correct browser for the task
		browserTaskPath(abbyyshot) {
				browserTypeArray	:=	{1:["Откройте ЯНДЕКС браузер", "Tasks\YandexTasks"], 2:[" ", "Tasks\FirefoxTasks"]}
	...
	;Build Synonym and NonSynonym Task path
		browserTaskFullPath(abbyyshot)	{
			...
			skipTasks			:=	["Зайдите (перейдите) в блог", "(перейдите) на форум", "Зайдите на сайт/блог/форум"]
			taskTypeArray		:=	{"synonym":"наберите любой СИНОНИМ слова", "nonsynonym":"Введите ключевое слово:", "stronglink":"Откройте браузер и перейдите на сайт", "directweb":"Пользователь обязан открыть удобный для него браузер", "directyoutube":"Откройте ролик в YouTube", "searchyoutube":"Откройте https://www.youtube.com и наберите ключевую фразу",  "yandexnews":"news.yandex", "googlenews":"Далее пройдите в раздел Новости"}
			...
			If InStr(abbyyshot, "Откройте ЯНДЕКС браузер") {
				...
				If InStr(abbyyshot, taskType)	{
					if ((keytas="synonym") or (keytas="nonsynonym")) {
						if(keytas="nonsynonym"){
							searchWords := this.FFWordExtract(abbyyshot, "Введите ключевое слово:", "»",  "«", "non")
